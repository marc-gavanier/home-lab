...
| ID | What reopened it |
|-----|--------------------------------------------------------------|
| C26 | A credential reaching **a trace**. #259 removed the Traefik access log's 400-599 filter on 2026-08-28 so the log could answer who is present; Vaultwarden's SignalR endpoint carries its bearer token in the query string. 21 lines, 7 distinct live tokens. The filter is exactly what made them invisible to the 4-axis sweep of 2026-08-22. Tracked as **#287**, written deliberately vague — the repo is public and the condition is live |
