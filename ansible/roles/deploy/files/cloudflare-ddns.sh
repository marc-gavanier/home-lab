...
# --- Zone id ---
ZID="$(curl -fsS -m 10 "${AUTH[@]}" "$API/zones?name=$CF_ZONE" | jq -r '.result[0].id // empty')"
[ -n "$ZID" ] || { log "ERROR: zone '$CF_ZONE' not found (token scope?)"; notify down "zone lookup failed"; exit 1; }

# --- Record lookup ---
REC="$(curl -fsS -m 10 "${AUTH[@]}" "$API/zones/$ZID/dns_records?type=A&name=$CF_RECORD")"
...
if [ -z "$RID" ]; then
    ...
    if echo "$OUT" | jq -e '.success' >/dev/null 2>&1; then
        ...
        log "ERROR: create failed: $(echo "$OUT" | jq -c '.errors')"; notify down "create failed"; exit 1
    fi
    exit 0
fi
...
OUT="$(curl -fsS -m 10 "${AUTH[@]}" -X PATCH "$API/zones/$ZID/dns_records/$RID" --data "{\"content\":\"$IP\"}")"
if echo "$OUT" | jq -e '.success' >/dev/null 2>&1; then
    log "UPDATED $CF_RECORD $CUR -> $IP"; notify up "updated $IP"
else
    log "ERROR: update failed: $(echo "$OUT" | jq -c '.errors')"; notify down "update failed"; exit 1
fi
