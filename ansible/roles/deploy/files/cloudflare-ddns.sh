...
notify() {
    ...
    # A TIMEOUT IS NOT A LOSS. Until 2026-09-13 every non-zero curl status wrote
    # `kuma-push-failed: this report reached nobody`, a sentence this side of the
    # wire cannot justify: whether the beat LANDED is decided by the server, while
    # curl only reports whether a reply came back inside its own budget. Measured
    # on the netdata emitter that day — Kuma committed the beat at 23:25:29.334
    # UTC, the script wrote "reached nobody" exactly TIMEOUT later, payloads
    # byte-identical. The full reasoning lives once, in homelab-netdata-kuma.sh.
    # curl exit 28 therefore gets `kuma-push-unconfirmed:`, which the live
    # assertion's anchored `--grep '^kuma-push-failed:'` does NOT match. Every
    # other status (22 on a 4xx/5xx, 7 refused, 6 DNS) is a beat that provably
    # did not land and keeps the original marker and the original words.
    #
    # `|| rc=$?` rather than reading PIPESTATUS after a bare pipeline: two of the
    # ten emitters run under `set -e`, where an unguarded failing pipeline would
    # abort the whole script — the guard the old `|| echo` provided by accident.
    # The verdict stays advisory, as it was: every branch leaves the status 0,
    # because what failed is the report, not the check it carries.
    local rc=0
    printf 'url = "%s"\n' "$url" |
