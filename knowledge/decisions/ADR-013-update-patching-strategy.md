...
- The exposed attack surface is tiny: **80/443** (Traefik, `vpn-only` → 403 to
  all internet traffic) and **51820/udp** (WireGuard, silent drop). SSH is not
  internet-exposed. The only remotely-reachable pre-auth kernel code is the
