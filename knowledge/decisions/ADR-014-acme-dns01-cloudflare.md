...
**Date**: 2026-07-20
**Status**: accepted, deployed and verified — 18 certificates in `acme.json` on the day it was accepted, expiry watched at 21 days by `homelab-health.sh` since #157. The count grows with the routers; the health message carries the current one.

## Context

Traefik issued Let's Encrypt certificates via the **HTTP-01** challenge: LE must
reach `http://<host>.example.com/.well-known/...` from the internet. That
...
- **Every** service subdomain (`drive`, `vault`, `photos`, `videos`, …) needs a
  **public A record** pointing at the home IP, purely so ACME can validate. So
  `dig vault.example.com` publicly reveals the home IP and the service list,
...
- The Traefik **dashboard** host (`proxy.example.com`) had no public A record,
...
- Service subdomains no longer need a public A record for certificates → the
  public A records can be removed (keep only `vpn.example.com`, needed to
...
- Port **80** is no longer needed for ACME → the ISP router forward can drop to
  ...
  redirect over VPN.
- The dashboard (`proxy.example.com`) now gets a valid cert, and a split-DNS
...
- **`*.example.com` wildcard via DNS-01**: one cert for all, but one private key
